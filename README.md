...
| Skill                | 用途                                                                                                                                                               |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `setup-dev-workflow` | 为目标仓库配置工单跟踪、手动验证约定（工控/纯桌面端）、文档布局；首次使用前运行一次。支持团队仓库个人模式（`AGENTS.local.md` + `.git/info/exclude`，不改团队文件） |
...
### 编写中（`skills/in-progress/`）

| Skill       | 用途                                                                                                                                                                                                  |
| ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `deep-read` | 深读代码库选定模块，产出经闭卷考试验证（ABC 循环）的认知文档，落盘目标仓库 `docs/analysis/` 并注册 AGENTS 指针；支持 `.sln` 解决方案模式（拓扑排序串行阅读）。移植自 deep-code-reader                   |
...
| [CiferaTeam/deep-code-reader](https://github.com/CiferaTeam/deep-code-reader)（`ref/deep-code-reader/`，MIT） | 深读代码库产出认知 skill 的工具，核心是 ABC 闭卷考试验证循环（A 读源码写文档、B 读源码出题、C 只看文档答题）。`deep-read` 移植自这里，产出改为仓库内 `docs/analysis/` 文档 + AGENTS 指针，并增加 `.sln` 解决方案模式。                                |
| [obra/superpowers](https://github.com/obra/superpowers)（`ref/superpowers/`，MIT）              | Claude Code 的 skill 合集，deep-code-reader 依赖其中的 writing-skills 格式规范；本仓库以自有 `writing-for-agents` 替代该依赖。                                                                                                                       |
